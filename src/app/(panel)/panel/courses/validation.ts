...
interface InitialValuesParams {
    ...
    categoryId?: string;
    tagList?: string[];
    approximateEpisodeCount?: number | null;
    level?: number | null;
...
export const getInitialValues = ({
    ...
    const baseValues = {
        ...
        Tags: rest.tagList ?? [],
        ApproximateEpisodeCount: rest.approximateEpisodeCount ?? null,
    };
...
export const getValidationSchema = ({ isEdit = false }) => {
    const baseSchema = {
        ...
        ApproximateEpisodeCount: Yup.number()
            .nullable()
            .required("تعداد جلسه ها الزامی است"),
    ...
    // در حالت create (غیر از ویرایش) فیلدهای اضافی را اضافه کن
    if (!isEdit) {
        return Yup.object({
            ...baseSchema,
            Level: Yup.number()
