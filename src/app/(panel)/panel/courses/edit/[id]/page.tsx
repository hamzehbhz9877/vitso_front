...
const Page = () => {
    ...
    const {mutate, isPending} = useMutation({
        ...
        onSettled: async (_, error) => {
            if (!error) {
                queryClient.invalidateQueries({queryKey: ["coursess"]});
                handleClose();
            }
        }
    });
    ...
    const handleSubmit = (values) => {
        const data = objectToFormData({...values,ApproximateEpisodeCount:+values.ApproximateEpisodeCount});
    ...
    return (
        <div>
            <Formik
                initialValues={getInitialValues({isEdit: true, ...courses?.data})}
                    ...
                    <Form>
                        <div className="flex gap-5">
                            <div className={"w-full lg:w-[300px]"}>
                                <InputDemo
                                    defaultData={courses?.data.image}
                                    name={"Image"}
                                    title={"تصویر دوره"}
                                    onChange={(file) => {
                                    ...
                                    <div
                                        ...
                                        <div className={"flex flex-col gap-3"}>
                                            <CalendarHijriInput
                                                ...
                                                initialDate={courses?.data?.publishedAt ? parseJalaliDateToDate(courses?.data?.publishedAt?.split(" ")[0]) : undefined}
                                                initValue={courses?.data?.publishedAt?.split(" ")[0]}
                                            />
                                            <SimpleInput label="تعداد تقریبی جلسات" name="ApproximateEpisodeCount"
                                                         type="text"/>

                                        </div>

                                        <TextArea rows={7} label="توضیحات کوتاه " name="ShortDescription"/>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="my-[20px]">
                                <Editor
                                    defaultData={courses?.data.description}
                                    name={"description"}
