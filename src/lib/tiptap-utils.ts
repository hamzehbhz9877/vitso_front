...
export const handleImageUpload = (uploadUrl: string) => {
  return async (
      file: File,
    ...
    if (file.size > MAX_FILE_SIZE) {
      throw new Error(`حجم فایل بیشتر از حد مجاز است. (حداکثر ${(MAX_FILE_SIZE / (1024 * 1024)).toFixed(1)} مگابایت)`)
    }
    ...
    try {
      const response = await instantClient.post(uploadUrl, formData, {
        signal: abortSignal,
