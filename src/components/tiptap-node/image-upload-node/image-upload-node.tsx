 ...
 *
 * @description
 * Provides functionality to upload files with progress tracking, validation,
 * and the ability to cancel ongoing uploads. Handles multiple file uploads
 * and provides status updates.
 *
 * @param {UploadOptions} options - Configuration options for the file upload process
 * @returns {Object} Object containing methods and state for managing file uploads
 */
 ...
 *
 * @description
 * Renders a container that accepts files via drag-and-drop interaction or can contain
 * other interactive elements like file input buttons. When files are dropped or selected,
 * the onFile callback is triggered with the file list.
 *
 * @param {ImageUploadDragAreaProps} props - The component props
 * @param {Function} props.onFile - Callback function that receives the selected files
 * @param {React.ReactNode} [props.children] - Optional content to render inside the drag area
 *
 * @returns {JSX.Element} The rendered drag area component
 */
...
interface ImageUploadPreviewProps {
  /**
   * The File object representing the image to be previewed
   */
  file: File
  /**
   * Current upload progress as a percentage (0-100)
  ...
  status: "uploading" | "success" | "error"
  /**
   * Callback function to remove this file from the upload queue
   */
  onRemove: () => void
}

/**
 * A component that displays a preview of an image being uploaded
 *
 * @description
 * Renders a preview thumbnail of an uploading image along with status indicators,
 * progress information, and a remove button. The component visualizes different
 * states (uploading, success, error) and allows users to cancel/remove the upload.
 *
 * @param {ImageUploadPreviewProps} props - The component props
 * @param {File} props.file - The image file to preview
 * @param {number} props.progress - Current upload progress (0-100)
 * @param {"uploading" | "success" | "error"} props.status - Current upload status
 * @param {Function} props.onRemove - Callback to remove this file from upload queue
 *
  ...
  <>
    ...
    <div className="tiptap-image-upload-content">
      <span className="tiptap-image-upload-text">
        <em>برای آپلود کلیک کنید</em> یا عکس مورد نظر خود را در این قسمت رها کنید
      </span>
      <span className="tiptap-image-upload-subtext">
       حداکثر حجم آپلود فایل {maxSize / 1024 / 1024}MB.
      </span>
    </div>
  </>
)
